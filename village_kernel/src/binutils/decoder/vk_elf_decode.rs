//###########################################################################
// vk_elf_decode.rs
// The specific implementation of functions related to elf decode
//
...
impl ElfDecoder {
    ...
    // Decode
    fn decode(&mut self, data: Vec<u8>, dynamic: u32) -> bool {
        // Set data
        self.data = data;
...
impl ElfDecoder {
    // Init
    pub fn init(&mut self, path: &str, data: Vec<u8>, dynamic: u32) -> bool {
        self.filename = path.to_string();

        if !self.decode(data, dynamic) {
            return false;
        }
