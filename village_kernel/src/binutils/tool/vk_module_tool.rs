    ...
    pub fn install(&mut self, filename: &str) -> bool{
        ...
        for module in self.mods.iter_mut() {
            if module.get_filename() == filename {
                kernel().debug().output(DebugLevel::Lv2, &format!("{} module has already been installed", filename));
                return true;
            }
        }
        ...
        // Install failed
        kernel().debug().error(&format!("{} module install failed", filename));
        false
    }
    ...
    pub fn uninstall(&mut self, filename: &str) -> bool {
        ...
        self.mods.retain_mut(|module| {
            if module.get_filename() == filename {
                is_unistall = true;
                module.fini_array();
                kernel().debug().output(DebugLevel::Lv2, &format!("{} module uninstall successful", filename));
                false
            } else {
                true
            }
        ...
        if !is_unistall {
            kernel().debug().error(&format!("{} module not found", filename));
            return false;
        }
        true
    }
